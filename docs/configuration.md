...
`username`, `password`, and `s3://` URLs), but it is not part of normal upstream
routing. It is only used after normal candidates are unavailable and its
responses are not persisted as route winners.
